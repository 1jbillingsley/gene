...
def demo_conversation_context():
    ...
    context = get_conversation_context()
    print(
        f"Custom context added: {context.get('user_preference')}, {context.get('location')}"
    )
...
def demo_error_handling():
    ...
    rate_limit_error = OpenAIErrorFactory.agent_rate_limit_error(
        retry_after=30, agent_id="rate_limited_agent"
    )
...
def demo_convenience_functions():
    ...
    client1 = create_mock_agents_sdk_client(
        response="Response from convenience function", agent_id="convenience_agent"
    )
