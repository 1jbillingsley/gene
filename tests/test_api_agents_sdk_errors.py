...
class TestAPIAgentsSDKErrorHandling:
    ...
    def test_agent_rate_limit_error_response(self):
        ...
        error_client = ErrorMockOpenAIClient(
            OpenAIErrorFactory.agent_rate_limit_error(
                retry_after=120, agent_id="agent_456"
            )
        )
    ...
    def test_agent_connection_error_response(self):
        ...
        assert (
            data["error"]
            == "Unable to connect to OpenAI Agents SDK service. Please check your internet connection and try again."
        )
    ...
    def test_agent_timeout_error_response(self):
        ...
        assert (
            data["error"]
            == "Agents SDK request timed out. The service may be experiencing high load. Please try again."
        )
    ...
    def test_agent_conversation_error_response(self):
        ...
        assert (
            data["error"]
            == "Agent conversation context error. The conversation state may be corrupted. Please start a new conversation."
        )
    ...
    def test_agent_processing_failure_response(self):
        ...
        assert (
            data["error"]
            == "Agent processing failed. The AI service may be temporarily unavailable. Please try again."
        )
    ...
    def test_agent_service_unavailable_response(self):
        ...
        assert (
            data["error"]
            == "OpenAI Agents SDK service is temporarily unavailable. Please try again later."
        )
    ...
    def test_retry_after_header_only_for_rate_limits(self):
        ...
        set_client(
            ErrorMockOpenAIClient(
                OpenAIErrorFactory.agent_rate_limit_error(retry_after=90)
            )
        )
        response = client.post("/messages", json={"body": "test message"})
        assert response.status_code == 429
        ...
        set_client(
            ErrorMockOpenAIClient(OpenAIErrorFactory.agent_authentication_error())
        )
        response = client.post("/messages", json={"body": "test message"})
        assert response.status_code == 401
