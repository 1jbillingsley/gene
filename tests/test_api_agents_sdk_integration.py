...
class MockAgentsSDKClient:
    ...
    def __init__(
        self, agent_id: str = "test_agent_123", conversation_id: str = "conv_456"
    ):
...
class MockAgentsSDKClientWithError:
    ...
    def __init__(
        self, error_type: str = "rate_limit", agent_id: str = "error_agent_789"
    ):
        self._agent_id = agent_id
...
class TestAgentsSDKResponseHeaders:
    ...
    def test_agents_sdk_response_includes_conversation_headers(
        self, client, mock_agents_sdk_client
    ):
        """Test that Agents SDK responses include conversation context headers."""
        set_client(mock_agents_sdk_client)

        response = client.post(
            "/messages", json={"body": "Test message for agents SDK"}
        )
    ...
    def test_agents_sdk_response_includes_model_header(
        self, client, mock_agents_sdk_client
    ):
...
class TestAgentsSDKErrorHandling:
    ...
    def test_agents_sdk_rate_limit_error_with_headers(self, client):
        ...
        data = response.json()
        assert (
            data["error"] == "Agents SDK rate limit exceeded. Please try again later."
        )
    ...
    def test_agents_sdk_conversation_error_with_details(self, client):
        ...
        assert (
            data["error"]
            == "Agent conversation context error. The conversation state may be corrupted."
        )
    ...
    def test_agents_sdk_internal_error_with_details(self, client):
        ...
        assert (
            data["error"]
            == "Unexpected agent processing error. Please try again or contact support."
        )
...
class TestAgentsSDKResponseFormat:
    ...
    def test_agents_sdk_response_format_consistency(
        self, client, mock_agents_sdk_client
    ):
        ...
        response = client.post(
            "/messages", json={"body": "Test message for format check"}
        )
    ...
    def test_agents_sdk_metadata_includes_conversation_context_size(
        self, client, mock_agents_sdk_client
    ):
        ...
        assert (
            metadata["conversation_context_size"] == 1
        )  # Only conversation_id in mock
...
class TestAgentsSDKConversationContext:
    ...
    def test_conversation_context_persistence_across_requests(self, client):
        ...
        mock_client = MockAgentsSDKClient(
            agent_id="persistent_agent", conversation_id="persistent_conv"
        )
...
class TestAgentsSDKIntegrationScenarios:
    ...
    def test_complete_agents_sdk_message_processing_flow(
        self, client, mock_agents_sdk_client
    ):
    ...
    def test_agents_sdk_with_tool_fallback_precedence(
        self, client, mock_agents_sdk_client
    ):
