...
class MockAgentsSDKClient:
    ...
    def __init__(
        ...
        self._agent.usage = {"total_tokens": 150, "prompt_tokens": 50, "completion_tokens": 100}
    ...
    def _handle_tool_suggestions(self, prompt: str) -> Optional[str]:
        ...
        if "suggest echo" in prompt_lower:
            ...
            tool_input = prompt[prompt.lower().find("suggest echo") + len("suggest echo"):].strip()
            if tool_input:
                # Simulate tool execution within agent context
                tool_result = tool_input  # Echo behavior
                self.tool_executions.append({"tool": "echo", "input": tool_input, "output": tool_result})
        ...
        # Simulate agent suggesting reverse tool
            ...
            tool_input = prompt[prompt.lower().find("suggest reverse") + len("suggest reverse"):].strip()
            if tool_input:
                # Simulate tool execution within agent context
                tool_result = tool_input[::-1]  # Reverse behavior
                self.tool_executions.append({"tool": "reverse", "input": tool_input, "output": tool_result})
                self._conversation_context["last_tool_used"] = {
                    "tool_name": "reverse",
                    "input": tool_input,
                    "output": tool_result
    ...
    def reset(self) -> None:
        ...
        conversation_id = self._conversation_context.get("conversation_id", "mock_conv_456")
        self._conversation_context = {
            "conversation_id": conversation_id,
            "turn_count": 0,
            "created_at": "2024-01-01T00:00:00Z"
...
class OpenAIErrorFactory:
    ...
    @staticmethod
    def agent_rate_limit_error(retry_after: int = 60, agent_id: str = "agent_123") -> OpenAIException:
