...
class MockAgentForE2E:
    ...
    def process_message(
        self, message: str, context: dict = None, available_tools: list = None
    ):
        ...
        if "help with echo" in message.lower():
            return Mock(
                content="I can help you with that. You might want to use echo tool for echoing text."
            )
        elif "help with reverse" in message.lower():
            return Mock(
                content="For reversing text, you should use reverse tool with your input."
            )
        elif "what tools" in message.lower():
            tool_names = [tool.get("name", "unknown") for tool in self.last_tools]
            return Mock(
                content=f"I have access to these tools: {', '.join(tool_names)}. I can help you use them effectively."
            )
        else:
...
class TestToolAgentEndToEnd:
    ...
    def test_agent_tool_awareness_through_api(self):
        ...
        with patch("gene.openai_client.Agent", MockAgentForE2E):
            ...
            response = client.post(
                "/messages", json={"body": "what tools are available?"}
            )
            assert response.status_code == 200
            data = response.json()
    ...
    def test_agent_suggests_tool_usage(self):
        ...
        with patch("gene.openai_client.Agent", MockAgentForE2E):
            ...
            response = client.post(
                "/messages", json={"body": "I need help with echo functionality"}
            )
            assert response.status_code == 200
            data = response.json()
