...
from gene.openai_client import _OpenAIWrapper, AgentsSDKWrapper, get_client, create_client, OpenAIError, OpenAIException
...
class TestOpenAIWrapper:
    ...
    def test_complete_rate_limit_error(self, mock_openai_module, mock_openai_class):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create.side_effect = MockRateLimitError("Rate limit exceeded", retry_after=120)
        mock_openai_class.return_value = mock_client
    ...
    def test_complete_connection_error(self, mock_openai_module, mock_openai_class):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create.side_effect = MockAPIConnectionError("Connection failed")
        mock_openai_class.return_value = mock_client
    ...
    def test_complete_timeout_error(self, mock_openai_module, mock_openai_class):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create.side_effect = MockAPITimeoutError("Request timed out")
        mock_openai_class.return_value = mock_client
    ...
    def test_complete_bad_request_error(self, mock_openai_module, mock_openai_class):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create.side_effect = MockBadRequestError("Bad request")
        mock_openai_class.return_value = mock_client
...
class TestOpenAIErrorHandling:
    ...
    @patch('gene.openai_client.OpenAI')
    @patch('gene.openai_client.openai')
    def test_rate_limit_with_default_retry_after(self, mock_openai_module, mock_openai_class):
...
class TestAgentsSDKWrapper:
    ...
    def test_complete_with_conversation_context(self, mock_agent_class):
        ...
        # Verify conversation context was updated
        assert wrapper._conversation_context == {"conversation_id": "test_123", "turn": 2}
    ...
    def test_complete_agent_rate_limit_error(self, mock_agent_class):
        ...
        mock_agent = Mock()
        mock_agent.process_message.side_effect = MockAgentRateLimitError("Agent rate limit exceeded", retry_after=90)
        mock_agent_class.return_value = mock_agent
    ...
    def test_complete_agent_authentication_error(self, mock_agent_class):
        ...
        mock_agent = Mock()
        mock_agent.process_message.side_effect = MockAgentAuthError("Invalid agent credentials")
        mock_agent_class.return_value = mock_agent
        ...
        assert "Invalid OpenAI Agents SDK credentials or insufficient permissions" == error.message
    ...
    def test_complete_unexpected_agent_error(self, mock_agent_class):
        ...
        assert "Unexpected agent processing error. Please try again or contact support if the issue persists." == error.message
