...
class TestAgentsSDKErrorHandling:
    ...
    def test_conversation_error_handling(self, mock_agent_class):
        ...
        mock_agent = Mock()
        mock_agent.process_message.side_effect = Exception("Conversation context corrupted")
        mock_agent_class.return_value = mock_agent
    ...
    def test_service_unavailable_error_handling(self, mock_agent_class):
        ...
        mock_agent = Mock()
        mock_agent.process_message.side_effect = Exception("Service temporarily unavailable")
        mock_agent_class.return_value = mock_agent
...
class TestAgentsSDKErrorFactory:
    ...
    def test_agent_rate_limit_error_creation(self):
        ...
        assert error.message == "Agents SDK rate limit exceeded. Please try again later."
        assert error.status_code == 429
...
class TestErrorClassNameMatching:
    ...
    def test_error_message_content_matching(self, mock_agent_class):
        ...
        mock_agent = Mock()
        mock_agent.process_message.side_effect = Exception("quota exceeded for requests")
        mock_agent_class.return_value = mock_agent
