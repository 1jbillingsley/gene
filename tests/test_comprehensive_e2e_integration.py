...
from gene.agent import set_client, process_message, get_conversation_context, update_conversation_context, clear_conversation_context
from gene.api import app
from gene.config import Settings, load_env_file
from gene.models import Message, ActionResult
from gene.openai_client import get_client, AgentsSDKWrapper, OpenAIError, OpenAIException


class MockAgentsSDKClient:
    """Comprehensive mock Agents SDK client for end-to-end testing."""
    
    def __init__(self, agent_id: str = "e2e_agent_123", conversation_id: str = "e2e_conv_456", model: str = "gpt-4"):
...
class TestComprehensiveEndToEndFlow:
    ...
    def test_complete_api_to_agents_sdk_flow(self):
        ...
        assert data["reply"] == "Hello! I'm processing your first message: Hello, Agents SDK!"
    ...
    def test_conversation_context_persistence_across_multiple_api_calls(self):
        ...
        assert data1["metadata"]["conversation_context_size"] == 3  # conversation_id + message_count + last_message
        ...
        assert response2.headers["X-Conversation-ID"] == "e2e_conv_456"  # Same conversation
        ...
        assert data3["metadata"]["conversation_context_size"] == 3  # Still 3 keys, but values updated
        
        # Verify conversation history was maintained
        assert len(mock_client.conversation_history) == 3
        assert mock_client.conversation_history == ["Start conversation", "Continue conversation", "Third message"]
...
class TestResponseFormatConsistency:
    ...
    def test_response_format_consistency_agents_sdk(self):
        ...
        metadata = data["metadata"]
        required_fields = ["source", "agent_id", "conversation_id", "structured_output", "processing_time", "model", "tokens_used"]
    ...
    def test_error_response_format_consistency(self):
        ...
        error_client = MockAgentsSDKClientWithErrors(error_scenario="conversation_error")
        set_client(error_client)
...
class TestConfigurationAndInitialization:
    ...
    def test_env_file_loading_integration(self):
        ...
        original_env = {}
        env_keys = ["OPENAI_API_KEY", "OPENAI_MODEL", "OPENAI_AGENT_ID", 
                   "OPENAI_CONVERSATION_MEMORY", "OPENAI_STRUCTURED_OUTPUT",
...
class TestConversationContextManagement:
    ...
    def test_conversation_context_api_functions(self):
        ...
        update_conversation_context({"user_preference": "verbose", "session_id": "test_123"})
        ...
        assert updated_context["conversation_id"] == "e2e_conv_456"  # Original preserved
    ...
    def test_conversation_context_persistence_during_processing(self):
        ...
        # Set initial context
        update_conversation_context({"user_id": "test_user", "preferences": {"lang": "en"}})
