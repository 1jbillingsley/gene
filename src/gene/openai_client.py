...
class _OpenAIWrapper:
    ...
    def _handle_api_error(self, error: Exception) -> str:
        ...
        if error_class_name == 'RateLimitError':
            ...
            retry_after = getattr(error, 'retry_after', None) or 60  # Default 60 seconds
            error_info = OpenAIError(
...
class AgentsSDKWrapper:
    ...
    def _handle_tool_suggestions(self, response, original_prompt: str):
        ...
        try:
            ...
            if "use echo tool" in response_text.lower() or "echo tool can" in response_text.lower():
                from .tools import get_tool
                # Extract the part after "suggest echo" for tool execution
                if "suggest echo" in original_prompt.lower():
                    tool_input = original_prompt[original_prompt.lower().find("suggest echo") + len("suggest echo"):].strip()
            ...
            elif "use reverse tool" in response_text.lower() or "reverse tool can" in response_text.lower():
                from .tools import get_tool
                # Extract the part after "suggest reverse" for tool execution
                if "suggest reverse" in original_prompt.lower():
                    tool_input = original_prompt[original_prompt.lower().find("suggest reverse") + len("suggest reverse"):].strip()
    ...
    def _create_agent(self) -> Agent:
        ...
        try:
            ...
            self._agent_id = getattr(agent, 'id', None) or getattr(agent, 'agent_id', None)
            
            logger.info(f"Created OpenAI Agent with model: {self._model}, agent_id: {self._agent_id}, tools: {len(available_tools)}")
            return agent
    ...
    def _handle_agent_error(self, error: Exception) -> None:
        ...
        if ('RateLimitError' in error_class_name or 
        ...
        elif ('AuthenticationError' in error_class_name or 
              'PermissionError' in error_class_name or
        ...
        elif ('ConnectionError' in error_class_name or 
              'NetworkError' in error_class_name or
        ...
        elif ('TimeoutError' in error_class_name or 
        ...
        elif ('ConversationError' in error_class_name or 
              'ContextError' in error_class_name or
              'conversation' in error_message or
              'context' in error_message or
              'memory' in error_message or
              'session' in error_message):
        ...
        elif ('AgentError' in error_class_name or 
              'ProcessingError' in error_class_name or
              'agent failed' in error_message or
              'processing failed' in error_message or
              'agent unavailable' in error_message):
        ...
        elif ('ValidationError' in error_class_name or 
              'BadRequestError' in error_class_name or
        ...
        # Handle service unavailable errors
        elif ('ServiceUnavailableError' in error_class_name or 
            ...
            error_info = OpenAIError(
                message=f"Unexpected agent processing error. Please try again or contact support if the issue persists.",
                status_code=500,
...
def get_client() -> OpenAIClient | None:
    ...
    try:
        ...
        logger.warning(f"Failed to create Agents SDK client: {e}; running in placeholder mode")
        return None
