...
def get_conversation_context() -> dict:
    ...
    if isinstance(_client, AgentsSDKWrapper) or hasattr(
...
def update_conversation_context(context: dict) -> None:
    ...
    if _client is None:
        logger.warning(
            "Cannot update conversation context: Agents SDK client not available"
        )
        return

    # Check if client has conversation context (either real AgentsSDKWrapper or mock)
    if not (
        isinstance(_client, AgentsSDKWrapper)
        or hasattr(_client, "_conversation_context")
    ):
        logger.warning(
            "Cannot update conversation context: Agents SDK client not available"
        )
    ...
    if not isinstance(context, dict):
        logger.warning(
            "Cannot update conversation context: context must be a dictionary"
        )
        return
...
def clear_conversation_context() -> None:
    ...
    if _client is None:
        logger.warning(
            "Cannot clear conversation context: Agents SDK client not available"
        )
        return
...
def process_message(message: Message) -> ActionResult:
    ...
    if _client is None:
        ...
        return ActionResult(
            reply=reply,
            metadata={"source": "placeholder", "processing_time": processing_time},
        )
        ...
        try:
            ...
            if isinstance(_client, AgentsSDKWrapper) or hasattr(_client, "_agent_id"):
            ...
            return ActionResult(reply=reply, metadata=metadata)
        except OpenAIException as e:
            # Enhanced error propagation with Agents SDK error information
            logger.error(
                f"Agents SDK error during message processing: {e.error.message}"
            )
